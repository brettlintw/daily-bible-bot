...
# --- 1. 頁面配置 (極致一頁式無捲頁) ---
st.set_page_config(page_title="聖經控制台 V14.4", page_icon="🛡️", layout="centered", initial_sidebar_state="collapsed")
        ...
        while True:
            try:
                ...
                if should_trigger:
                    ...
                    try:
                        # 核心修改 1：換裝高階旗艦模型 gemini-1.5-pro，解鎖極致文字細膩度
                        ...
                        res = model.generate_content(
                            ...
                            generation_config=genai.types.GenerationConfig(
                                temperature=0.75, top_p=0.90, max_output_tokens=800 # 徹底放寬至 800 Token
                            )
                        )
...
# --- 4. UI 佈局 ---
st.markdown(f"<h1>🛡️ 聖經任務控制台+LINE推送 V14.4 <span class='status-tag'>🛰️ 衛星通訊正常</span></h1>", unsafe_allow_html=True)
st.caption(f"📅 {datetime.now(TZ_TW).strftime('%m/%d')} | 🚀 旗艦全頻寬解鎖版")
