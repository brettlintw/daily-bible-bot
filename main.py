...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 8px; height: 2.5rem; font-weight: 600; }
    .stTextArea>div>div>textarea { border-radius: 8px; font-size: 16px !important; height: 75px !important; }
    .stMultiSelect div[data-baseweb="select"] { border-radius: 8px; min-height: 2.2rem; }
...
# --- 3. UI 介面佈局 ---
st.title("🛡️ 聖經任務控制台")
st.caption(f"📅 {datetime.now().strftime('%m/%d')} | 🛰️ 安全連線 | v8.0-Stable")
...
# 🤖 AI 推送區 (參考歷史版本修正 404 報錯)
st.subheader("🤖 AI 智慧推送")
if st.button("✨ 啟動 AI 測試"):
    try:
        with st.spinner("系統生成中..."):
            # 修正：改用最穩定的模型標識符，避免 404 錯誤
            model = genai.GenerativeModel('gemini-1.5-flash')
            res = model.generate_content("請幫我從聖經中挑選一段適合今日的經文並給予一段溫和的啟示，總字數限制在80字內。")
            
            if res and res.text:
                line_api.push_message(LINE_ID, TextSendMessage(text=f"【AI測試】\n\n{res.text}"))
                st.toast("✨ 推送成功")
                st.success("任務已執行")
            else: st.error("生成內容異常")
